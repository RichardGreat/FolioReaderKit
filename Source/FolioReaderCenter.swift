...
open class FolioReaderCenter: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    private func configure(readerPageCell cell: FolioReaderPage?, atIndexPath indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if folioReader.currentFont == .original {
            if let resourse = book.resources.findByMediaType(.ttf) {
                classes = resourse.id
            } else if let resourse = book.resources.findByMediaType(.ttf1) {
                classes = resourse.id
            } else if let resourse = book.resources.findByMediaType(.ttf2) {
                classes = resourse.id
            } else if let resourse = book.resources.findByMediaType(.openType) {
                classes = resourse.id
            } else if let resourse = book.resources.findByMediaType(.woff) {
                classes = resourse.id
            }
        }
        
        print("==================================================")
        print("current font css id ", classes)
        print("==================================================")
